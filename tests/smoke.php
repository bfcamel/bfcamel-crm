...
require_once dirname( __DIR__ ) . '/src/CRM/ContactService.php';
...
check( "https://vk.com/example\n@example" === \BfCamel\CRM\CRM\ContactService::normalize_social_pages( array( 'https://vk.com/example', 'https://vk.com/example', '@example' ) ), 'Social pages are not normalized and de-duplicated.' );
...
$defaults = new ReflectionMethod( RoleManager::class, 'default_permissions' ); $defaults->setAccessible( true );
$administrator_permissions = $defaults->invoke( null, 'administrator' );
$subscriber_permissions = $defaults->invoke( null, 'subscriber' );
check( count( RoleManager::capability_keys() ) === count( array_filter( $administrator_permissions ) ), 'Administrator permissions are incomplete.' );
check( 0 === count( array_filter( $subscriber_permissions ) ), 'Non-administrator roles receive unsafe permissions by default.' );
check( ConsentService::field_available( array( 'type' => 'consent_personal_data' ) ), 'Consent fields must remain available without document URLs.' );
...
check( false !== strpos( $contacts, 'contact[city]' ) && false !== strpos( $contacts, 'contact[social_pages]' ), 'Manual city or social-page editing is missing.' );
check( false !== strpos( $contacts, "esc_html_e('City','bfcamel-crm')" ), 'The native city table column is missing.' );
...
check( false !== strpos( $submissions, 'function delete_submission' ), 'Submission deletion controller is missing.' );
check( false !== strpos( file_get_contents( $root . '/src/Forms/Repository.php' ), 'function delete_permanently' ), 'Protected form deletion is missing.' );
check( ! is_file( $root . '/src/Admin/DataEnhancements.php' ) && ! is_file( $root . '/assets/data-enhancements.js' ), 'Legacy JavaScript-only data enhancement layer is still present.' );
$exporter = file_get_contents( $root . '/src/Export/Exporter.php' );
check( false === strpos( $exporter, 'DataEnhancements' ), 'Contact export still depends on the removed enhancement layer.' );
$admin_js = file_get_contents( $root . '/assets/admin.js' );
check( false !== strpos( $admin_js, 'labels.keyHelp' ) && false !== strpos( $admin_js, 'labels.mappingHelp' ), 'Form key and CRM mapping guidance is missing.' );
