...
final class SubmissionHandler {
    ...
    private function validate_payload($schema,$posted){$payload=array();foreach((array)$schema as $field){$name=sanitize_key($field['name']??'');$type=sanitize_key($field['type']??'text');$required=!empty($field['required']);if(!$name||'html'===$type||!ConsentService::field_available($field))continue;$raw=isset($posted[$name])?wp_unslash($posted[$name]):'';$value=$this->sanitize_value($type,$raw,(array)($field['options']??array()));if(is_wp_error($value))return $value;if($required&&$this->is_empty($value))return new \WP_Error('bfcamel_crm_required',sprintf(__( 'Required field missing: %s', 'bfcamel-crm' ),$name));if('email'===$type&&''!==$value&&!is_email($value))return new \WP_Error('bfcamel_crm_email',__( 'Invalid email address.', 'bfcamel-crm' ));$payload[$name]=in_array($type,array('consent_personal_data','consent_marketing'),true)?($value?'1':''):$value;}return $payload;}
