...
final class TagService {
    public static function all( $scope = '' ) {
        ...
        $rows = $wpdb->get_results( "SELECT * FROM {$table} ORDER BY name ASC, id ASC" ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    ...
    private static function for_entity( $entity_type, $entity_id ) {
        ...
        return $wpdb->get_results($wpdb->prepare("SELECT t.* FROM {$tags} t INNER JOIN {$links} rel ON rel.tag_id=t.id WHERE rel.{$column}=%d ORDER BY t.name ASC,t.id ASC",absint($entity_id)));
    }
    ...
    private static function sync_entity( $entity_type, $entity_id, $raw_names ) {
        ...
        $existing=array_map('absint',(array)$wpdb->get_col($wpdb->prepare("SELECT tag_id FROM {$links} WHERE {$column}=%d",$entity_id)));
        foreach(array_diff($existing,$tag_ids) as $tag_id){ if(false===$wpdb->delete($links,array($column=>$entity_id,'tag_id'=>absint($tag_id)),array('%d','%d')))return self::database_error(__( 'Could not remove a tag.', 'bfcamel-crm' )); }
        $now=current_time('mysql'); foreach(array_diff($tag_ids,$existing) as $tag_id){ if(!$wpdb->insert($links,array($column=>$entity_id,'tag_id'=>absint($tag_id),'created_at'=>$now),array('%d','%d','%s')))return self::database_error(__( 'Could not add a tag.', 'bfcamel-crm' )); }
